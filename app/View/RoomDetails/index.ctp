<script type="text/javascript">
	function deleteRecord(id){
		...
		if(conf){
			window.location = "<?php echo $this->webroot; ?>room_details/delete/"+id;
		}
		return false;
	}
</script>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(".bulk_options_button, .bulk-action-submit").click(function(){
			...
			switch( action ){
				case 'delete':
							$("#admin_panel_form").append('<input type="hidden" name="process_action" value="delete" /><input type="hidden" name="process_model" value="RoomDetail" />').submit();
							break;
				default:
...
<div class="row">
	...
	<div class="col-md-offset-6">
        	<?php echo $this->Form->create('RoomDetailSearch', array('url'=>'/roomDetails/index/', 'class'=>'grid-search-form form-inline', 'autocomplete'=>'off')); ?>
	...
	<table class="table">
		...
		<tr>
			<th><input type="checkbox" class="bulk_action_check" /></th>
                            ...
                            <th><?php echo $this->Paginator->sort('room_number'); ?></th>
                            <th><?php echo $this->Paginator->sort('room_details'); ?></th>
		...
		<tbody>
        <?php if(count($roomDetails) > 0): ?>
<?php foreach ($roomDetails as $roomDetail): ?>
	<tr>
<td><input type="checkbox" class="item_id" name="item_id[]" value="<?php echo $roomDetail['RoomDetail']['id']; ?>" /></td>
		<td><?php echo h($roomDetail['RoomDetail']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($roomDetail['RoomType']['id'], array('controller' => 'room_types', 'action' => 'view', $roomDetail['RoomType']['id'])); ?>
		</td>
		<td><?php echo h($roomDetail['RoomDetail']['room_number']); ?>&nbsp;</td>
		<td><?php echo h($roomDetail['RoomDetail']['room_details']); ?>&nbsp;</td>
		<td><?php echo h($roomDetail['RoomDetail']['comments']); ?>&nbsp;</td>
		<td><?php echo h($roomDetail['RoomDetail']['created']); ?>&nbsp;</td>
		<td><?php echo h($roomDetail['RoomDetail']['modified']); ?>&nbsp;</td>
		<td><?php echo h($roomDetail['RoomDetail']['isactive']); ?>&nbsp;</td>
		<td><?php echo h($roomDetail['RoomDetail']['isdeleted']); ?>&nbsp;</td>
		<td class="actions action_column">
			<?php echo $this->Html->image("admin/edit.png", array("alt"=>"edit", "title"=>"Edit", "url"=>array('action' => 'edit', $roomDetail['RoomDetail']['id']))); ?>
			<?php echo $this->Form->postLink($this->Html->image("admin/delete.png", array('alt' => __('delete'), 'title'=>'Delete')), array('action' => 'delete', $roomDetail['RoomDetail']['id']), array('escape'=>FALSE, 'confirm'=>__('Are you sure you want to delete # %s?', $roomDetail['RoomDetail']['id']))); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php else: ?>

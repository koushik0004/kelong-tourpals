<script type="text/javascript">
	function deleteRecord(id){
		...
		if(conf){
			window.location = "<?php echo $this->webroot; ?>facilities/delete/"+id;
		}
		return false;
	}
</script>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(".bulk_options_button, .bulk-action-submit").click(function(){
			...
			switch( action ){
				case 'delete':
							$("#admin_panel_form").append('<input type="hidden" name="process_action" value="delete" /><input type="hidden" name="process_model" value="Facility" />').submit();
							break;
				default:
...
<div class="row">
	...
	<div class="col-md-offset-6">
        	<?php echo $this->Form->create('FacilitySearch', array('url'=>'/facilities/index/', 'class'=>'grid-search-form form-inline', 'autocomplete'=>'off')); ?>
			<label for="grid_column_id">Select Column</label>
            <?php 
				$col_opts = array(
					'title'=>'Title',
					'isdeletd'=>'Deactive Fecilities', 
				);
			?>
            <?php $column = (isset($column) && !empty($column))?$column:''; ?>
            <?php echo $this->Form->input('UserSearch.grid_column', array('type'=>'select', 'options'=>$col_opts, 'empty'=>'Active facilities', 'id'=>'grid_column_id', 'class'=>'bulk-search-select', 'label'=>false, 'div'=>false, 'name'=>'grid_column', 'selected'=>$column));?>
...
<div class="grid-table">
	<form id="admin_panel_form" action="<?php echo $this->webroot; ?>facilities/bulk_actions" method="post">
	<table class="table">
		<thead>
		<tr>
			<th><!--<input type="checkbox" class="bulk_action_check" />--></th>
                            ...
                            <th><?php echo $this->Paginator->sort('title_alias'); ?></th>
                            <th><?php echo $this->Paginator->sort('icon_path'); ?></th>
                            <th><?php echo $this->Paginator->sort('created'); ?></th>
                            <th><?php echo $this->Paginator->sort('updated'); ?></th>
                            <th><?php echo $this->Paginator->sort('isactive'); ?></th>
                            <th><?php echo $this->Paginator->sort('isdeletd'); ?></th>
            			<th>Action</th>
		</tr>
		</thead>
		<tbody>
...
<?php foreach ($facilities as $facility): ?>
	<tr>
<td><input type="checkbox" class="item_id" name="item_id[]" value="<?php echo $facility['Facility']['id']; ?>" /></td>
		<td><?php echo h($facility['Facility']['id']); ?>&nbsp;</td>
		<td><?php echo h($facility['Facility']['title']); ?>&nbsp;</td>
		<td><?php echo h($facility['Facility']['title_alias']); ?>&nbsp;</td>
		<td><?php 
				$imgpath = 'uploads/50/'.$facility['Facility']['icon_path'];
				echo $this->Html->image($imgpath, array("alt"=>$facility['Facility']['icon_path'], "title"=>$facility['Facility']['title'])); 
			?>&nbsp;
        </td>
		<td><?php echo h($facility['Facility']['created']); ?>&nbsp;</td>
		<td><?php echo h($facility['Facility']['updated']); ?>&nbsp;</td>
		<td><?php echo h($facility['Facility']['isactive']); ?>&nbsp;</td>
		<td><?php echo h($facility['Facility']['isdeletd']); ?>&nbsp;</td>
		<td class="actions action_column">
			<?php echo $this->Html->image("admin/edit.png", array("alt"=>"edit", "title"=>"Edit", "url"=>array('action' => 'edit', $facility['Facility']['id']))); ?>
            
            <?php echo $this->Html->image("admin/delete.png", array('alt' => __('delete'), 'title'=>'Delete', "url"=>array('action' => 'delete', $facility['Facility']['id']))); ?>

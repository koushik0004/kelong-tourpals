...
<script type="text/javascript">
	function deleteRecord(id){
		...
		if(conf){
			window.location = "<?php echo $this->webroot; ?>places/delete/"+id;
		}
		return false;
	}
</script>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(".bulk_options_button, .bulk-action-submit").click(function(){
			...
			switch( action ){
				case 'delete':
							$("#admin_panel_form").append('<input type="hidden" name="process_action" value="delete" /><input type="hidden" name="process_model" value="Place" />').submit();
							break;
				default:
...
<div class="row">
	...
	<div class="col-md-offset-6">
        	<?php echo $this->Form->create('PlaceSearch', array('url'=>'/places/index/', 'class'=>'grid-search-form form-inline', 'autocomplete'=>'off')); ?>
...
'isdeleted'=>'Isdeleted', 
            ...
            <?php echo $this->Form->input('UserSearch.grid_column', array('type'=>'select', 'options'=>$col_opts, 'empty'=>'Active places', 'id'=>'grid_column_id', 'class'=>'bulk-search-select', 'label'=>false, 'div'=>false, 'name'=>'grid_column', 'selected'=>$column));?>
...
<div class="grid-table">
	<form id="admin_panel_form" action="<?php echo $this->webroot; ?>places/bulk_actions" method="post">
	<table class="table">
		<thead>
		<tr>
...
<?php foreach ($places as $place): ?>
	<tr>
<td><input type="checkbox" class="item_id" name="item_id[]" value="<?php echo $place['Place']['id']; ?>" /></td>
		...
		<td><?php echo h($place['Place']['title']); ?>&nbsp;</td>
		<td><?php echo h($place['Place']['created']); ?>&nbsp;</td>
		<td><?php echo h($place['Place']['updated']); ?>&nbsp;</td>
		<td><?php echo h($place['Place']['isactive']); ?>&nbsp;</td>
		<td><?php echo h($place['Place']['isdeleted']); ?>&nbsp;</td>
		<td class="actions action_column">
			<?php echo $this->Html->image("admin/edit.png", array("alt"=>"edit", "title"=>"Edit", "url"=>array('action' => 'edit', $place['Place']['id']))); ?>
			<?php echo $this->Form->postLink($this->Html->image("admin/delete.png", array('alt' => __('delete'), 'title'=>'Delete')), array('action' => 'delete', $place['Place']['id']), array('escape'=>FALSE, 'confirm'=>__('Are you sure you want to delete # %s?', $place['Place']['id']))); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php else: ?>

<script type="text/javascript">
	function deleteRecord(id){
		...
		if(conf){
			window.location = "<?php echo $this->webroot; ?>hotel_approved_ratings/delete/"+id;
		}
		return false;
	}
</script>
	...
	<table class="table">
		...
		<tr>
			<th><input type="checkbox" class="bulk_action_check" /></th>
                            ...
                            <th><?php echo $this->Paginator->sort('avarage_rating'); ?></th>
		...
		<tbody>
        <?php if(count($hotelApprovedRatings) > 0): ?>
<?php foreach ($hotelApprovedRatings as $hotelApprovedRating): ?>
	<tr>
<td><input type="checkbox" class="item_id" name="item_id[]" value="<?php echo $hotelApprovedRating['HotelApprovedRating']['id']; ?>" /></td>
		<td><?php echo h($hotelApprovedRating['HotelApprovedRating']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($hotelApprovedRating['Hotel']['title'], array('controller' => 'hotels', 'action' => 'view', $hotelApprovedRating['Hotel']['id'])); ?>
		</td>
		<td><?php echo h($hotelApprovedRating['HotelApprovedRating']['avarage_rating']); ?>&nbsp;</td>
		<td><?php echo h($hotelApprovedRating['HotelApprovedRating']['created']); ?>&nbsp;</td>
		<td><?php echo h($hotelApprovedRating['HotelApprovedRating']['updated']); ?>&nbsp;</td>
		<td><?php echo h($hotelApprovedRating['HotelApprovedRating']['isactive']); ?>&nbsp;</td>
		<td><?php echo h($hotelApprovedRating['HotelApprovedRating']['isdeleted']); ?>&nbsp;</td>
		<td class="actions action_column">
			<?php echo $this->Html->image("admin/edit.png", array("alt"=>"edit", "title"=>"Edit", "url"=>array('action' => 'edit', $hotelApprovedRating['HotelApprovedRating']['id']))); ?>
			<?php echo $this->Form->postLink($this->Html->image("admin/delete.png", array('alt' => __('delete'), 'title'=>'Delete')), array('action' => 'delete', $hotelApprovedRating['HotelApprovedRating']['id']), array('escape'=>FALSE, 'confirm'=>__('Are you sure you want to delete # %s?', $hotelApprovedRating['HotelApprovedRating']['id']))); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php else: ?>

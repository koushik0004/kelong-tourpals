...
<script type="text/javascript">
	function deleteRecord(id){
		...
		if(conf){
			window.location = "<?php echo $this->webroot; ?>hotels/delete/"+id;
		}
		return false;
	}
</script>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(".bulk_options_button, .bulk-action-submit").click(function(){
			...
			switch( action ){
				case 'delete':
							$("#admin_panel_form").append('<input type="hidden" name="process_action" value="delete" /><input type="hidden" name="process_model" value="Hotel" />').submit();
							break;
				default:
...
<div class="row">
	...
	<div class="col-md-offset-6">
        	<?php echo $this->Form->create('HotelSearch', array('url'=>'/hotels/index/', 'class'=>'grid-search-form form-inline', 'autocomplete'=>'off')); ?>
...
'isdeleted'=>'Isdeleted', 
            ...
            <?php echo $this->Form->input('UserSearch.grid_column', array('type'=>'select', 'options'=>$col_opts, 'empty'=>'Active hotels', 'id'=>'grid_column_id', 'class'=>'bulk-search-select', 'label'=>false, 'div'=>false, 'name'=>'grid_column', 'selected'=>$column));?>
...
<div class="grid-table">
	<form id="admin_panel_form" action="<?php echo $this->webroot; ?>hotels/bulk_actions" method="post">
	<table class="table">
		<thead>
		<tr>
...
<?php foreach ($hotels as $hotel): ?>
	<tr>
<td><input type="checkbox" class="item_id" name="item_id[]" value="<?php echo $hotel['Hotel']['id']; ?>" /></td>
		...
		<td><?php echo h($hotel['Hotel']['title']); ?>&nbsp;</td>
		<td><?php echo h($hotel['Hotel']['created']); ?>&nbsp;</td>
		<td><?php echo h($hotel['Hotel']['updated']); ?>&nbsp;</td>
		<td><?php echo h($hotel['Hotel']['isactive']); ?>&nbsp;</td>
		<td><?php echo h($hotel['Hotel']['isdeleted']); ?>&nbsp;</td>
		<td class="actions action_column">
			<?php echo $this->Html->image("admin/edit.png", array("alt"=>"edit", "title"=>"Edit", "url"=>array('action' => 'edit', $hotel['Hotel']['id']))); ?>
			<a href="javascript:void(0);" class="listing-delete-anchor"><img src="<?php echo $this->webroot.'img/admin/delete.png'; ?>" alt="delete" title="Delete"></a>
			<input type="hidden" class="listing-delete-location" value="<?php echo $this->webroot.'hotels/delete/'.$hotel['Hotel']['id']; ?>">
		</td>
	</tr>
<?php endforeach; ?>
<?php else: ?>
